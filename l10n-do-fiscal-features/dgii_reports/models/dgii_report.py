...
class DgiiReport(models.Model):
    ...
    def _compute_607_fields(self):
        for rec in self:
            ...
            for inv in sale_line_ids:
                data["sale_records"] += 1
                data["sale_invoiced_amount"] += (
                    inv.invoiced_amount if self._is_same_period(inv) else 0
                )
                data["sale_invoiced_itbis"] += (
                    inv.invoiced_itbis if self._is_same_period(inv) else 0
                )
    ...
    csmr_cash = fields.Monetary("Cash in Consumer", copy=False)
    csmr_bank = fields.Monetary("Check / Transfer / Deposit in Consumer", copy=False)
    csmr_card = fields.Monetary("Credit Card / Debit Card in Consumer", copy=False)
    csmr_credit = fields.Monetary("Credit in Consumer", copy=False)
    csmr_bond = fields.Monetary("Gift certificates or vouchers in Consumer", copy=False)
    csmr_swap = fields.Monetary("Swap in Consumer", copy=False)
    csmr_others = fields.Monetary("Other Sale Forms in Consumer", copy=False)
    ...
    def _get_pending_invoices(self, types):
        ...
        invoice_ids = (
            ...
            .search(
                [
                    ("l10n_do_fiscal_status", "=", "normal"),
                    ("payment_state", "!=", "not_paid"),
                    ("l10n_do_payment_date", "<=", start_date),
                    ("company_id", "=", self.company_id.id),
                    ("move_type", "in", types),
                ]
            )
            .filtered(
                lambda inv: self.get_date_tuple(inv.l10n_do_payment_date)
    ...
    def _get_invoices(self, states, payment_states, types):
        """
        ...
        invoice_ids = (
            ...
            .filtered(
                lambda inv: (
                    ...
                    and inv.l10n_latam_document_type_id.doc_code_prefix != "IMP"
                )
                or (inv.journal_id.l10n_latam_use_documents is True)
            )
        )
    ...
    def _include_in_current_report(self, invoice):
        ...
        :return: boolean
        """
        if not invoice.l10n_do_payment_date:
            return False

        l10n_do_payment_date = invoice.l10n_do_payment_date
        period = dt.strptime(self.name, "%m/%Y")
        same_minor_period = (l10n_do_payment_date.year, l10n_do_payment_date.month) <= (
            period.year,
            period.month,
        )

        return True if (l10n_do_payment_date and same_minor_period) else False

    def _is_exterior(self, invoice):
        return bool(invoice.l10n_latam_document_type_id.l10n_do_ncf_type == "exterior")
    ...
    def _compute_606_data(self):
        for rec in self:
            ...
            for inv in invoice_ids:
                inv.l10n_do_fiscal_status = (
                    "blocked"
                    if not inv.l10n_do_fiscal_status
                    else inv.l10n_do_fiscal_status
                )
                line += 1
                rnc_ced = (
                    self.formated_rnc_cedula(inv.partner_id.vat)
                    if inv.l10n_latam_document_type_id.l10n_do_ncf_type
                    not in ["exterior", "minor"]
                ...
                values = {
                    ...
                    "payment_date": inv.l10n_do_payment_date
                    if show_payment_date
                    else False,
                    "service_total_amount": inv.l10n_do_service_total_amount,
                    "good_total_amount": inv.l10n_do_good_total_amount,
                    "invoiced_amount": inv.amount_untaxed_signed,
                    "invoiced_itbis": inv.l10n_do_invoiced_itbis,
                    "proportionality_tax": inv.l10n_do_proportionality_tax,
                    "cost_itbis": inv.l10n_do_cost_itbis,
                    "advance_itbis": inv.l10n_do_advance_itbis,
                    "purchase_perceived_itbis": 0,  # F`alta computar en la fact
                    "purchase_perceived_isr": 0,  # Falta computarlo en la fact
                    "isr_withholding_type": inv.l10n_do_isr_withholding_type
                    if not self._is_exterior(inv)
                    else False,
                    "withholded_itbis": inv.l10n_do_withholded_itbis
                    if show_payment_date
                    else 0,
                    "income_withholding": inv.l10n_do_income_withholding
                    if show_payment_date and not self._is_exterior(inv)
                    else 0,
                    "selective_tax": inv.l10n_do_selective_tax,
                    "other_taxes": inv.l10n_do_other_taxes,
                    "legal_tip": inv.l10n_do_legal_tip,
                    "payment_type": inv.l10n_do_payment_form
                    if show_payment_date
                    else "04",
    ...
    def _convert_to_user_currency(self, base_currency, amount, date):
        ...
        ctx = context.copy()
        if base_currency != user_currency_id:
            amount = base_currency.with_context(ctx)._convert(
                amount, user_currency_id, company_id, date
            )
        return amount

    @api.model
    def include_payment(self, invoice_id, payment_id):
    ...
    def _get_sale_payments_forms(self, invoice_id):
        ...
        if invoice_id.move_type == "out_invoice":
            for payment in invoice_id._get_reconciled_info_JSON_values():
                ...
                if payment_id:
                    ...
                    refund_id = Invoice.browse(payment["move_id"])
                    if refund_id.move_type == "out_refund":
                        payments_dict["credit"] += abs(refund_id.amount_total_signed)
            payments_dict["credit"] += self._convert_to_user_currency(
                invoice_id.currency_id,
        ...
        return invoice_id._get_signed_data(payments_dict)
    ...
    def _get_607_operations_dict(self):
        return {
            ...
            "e-fiscal": {
                ...
                "qty": 0,
                "amount": 0,
                "name": "COMPROBANTE CRÉDITO FISCAL ELECTRÓNICO",
                "dgii_report_id": self.id,
            },
            "consumer": {
                "sequence": 3,
    ...
    def _is_same_period(self, invoice):
        """
        Evaluates that the invoice period is the same as the report
        :return: boolean
        """
        return invoice.invoice_date.strftime("%m/%Y") == self.name
        ...
        for rec in self:
            ...
            for inv in invoice_ids:
                ...
                values = {
                    ...
                    "invoice_date": inv.invoice_date,
                    "withholding_date": inv.l10n_do_payment_date
                    if (inv.move_type != "out_refund" and show_payment_date)
                    else False,
                    "invoiced_amount": inv.amount_untaxed_signed,
                    "invoiced_itbis": inv.l10n_do_invoiced_itbis,
                    "third_withheld_itbis": inv.l10n_do_third_withheld_itbis
                    if show_payment_date
                    else 0,
                    "perceived_itbis": 0,  # Pendiente
                    "third_income_withholding": inv.l10n_do_third_income_withholding
                    if show_payment_date
                    else 0,
                ...
                if str(values["fiscal_invoice_number"])[
                    -10:-8
                ] == "02" and self._is_same_period(inv):
                ...
                ) or inv.is_ecf_invoice:
                    excluded_line += 1
                    # Excluye las Facturas Electrónicas y todas las de Consumo
            ...
            invoice_ids = self._get_invoices(
                ...
                and (
                    (
                        ...
                        and inv.l10n_latam_document_type_id.l10n_do_ncf_type != "fiscal"
                    )
    ...
    def _compute_609_data(self):
        for rec in self:
            ...
            invoice_ids = self._get_invoices(
                ...
                lambda r: r.company_id.country_id == self.env.ref("base.do")
                and r.l10n_latam_document_type_id.l10n_do_ncf_type == "exterior"
            )
            line = 0
            report_data = ""
            for inv in invoice_ids:
                ...
                values = {
                    ...
                    "purchased_service_type": int(inv.l10n_do_service_type),
                    "service_type_detail": inv.l10n_do_service_type_detail.code,
                    "related_part": int(inv.partner_id.l10n_do_related),
                    "doc_number": inv.l10n_latam_document_number,
                    "doc_date": inv.invoice_date,
                    "invoiced_amount": inv.amount_untaxed_signed,
                    "isr_withholding_date": inv.l10n_do_payment_date
                    if inv.l10n_do_payment_date
                    else False,
                    "presumed_income": 0,  # Pendiente
                    "withholded_isr": inv.l10n_do_income_withholding
                    if inv.l10n_do_payment_date
                    else 0,
                    "invoice_id": inv.id,
                }
    ...
    def _has_withholding(self, inv):
        ...
        return (
            ...
            if any(
                [
                    inv.l10n_do_income_withholding,
                    inv.l10n_do_withholded_itbis,
                    inv.l10n_do_third_withheld_itbis,
                    inv.l10n_do_third_income_withholding,
                ]
            )
            else False
        )
    ...
    def _invoice_status_sent(self):
        for report in self:
            ...
            for inv in invoice_ids:
                if (
                    inv.state == "cancel" or inv.payment_state in ("paid", "in_payment")
                ) and self._include_in_current_report(inv):
                    inv.l10n_do_fiscal_status = "done"
                    continue

                if self._has_withholding(inv) or not inv.l10n_do_payment_date:
                    inv.l10n_do_fiscal_status = "normal"
                else:
                    inv.l10n_do_fiscal_status = "done"
    ...
    def update_pending_invoices(self):
        ...
        invoice_ids = self.env["account.move"].search(
            [
                ("payment_state", "!=", "not_paid"),
                ("l10n_do_fiscal_status", "=", "normal"),
                ("l10n_do_payment_date", "=", False),
            ]
        )
        invoice_ids.write({"l10n_do_fiscal_status": "done"})
    ...
    def get_607_tree_view(self):
        return {
            ...
            "domain": [("dgii_report_id", "=", self.id)],
            "context": {"search_default_current_period": 1},
        }
