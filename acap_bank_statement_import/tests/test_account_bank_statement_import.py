...
class AccountBankStatementImportTest(AccountTestInvoicingCommon):
    def test_001_acap_statement_import(self):
        """
        Check acap statement file is processed correctly
        """

        acap_file_path = get_module_resource(
            "acap_bank_statement_import", "acap_stmt_file", "acap_statement.csv"
        )
        acap_file = base64.b64encode(open(acap_file_path, "rb").read())
        ...
        import_wizard = (
            ...
            .create(
                {
                    "attachment_ids": [
                        (0, 0, {"name": "acap_stmt.csv", "datas": acap_file})
                    ]
                }
            )
        )
    ...
    def test_002_acap_statement_file_validation(self):
        """
        Check acap file format validation works correctly
        """

        acap_file_path = get_module_resource(
            "acap_bank_statement_import", "acap_stmt_file", "bad_acap_statement.csv"
        )
        acap_file = open(acap_file_path, "r").read()

        self.assertFalse(
            self.env["account.bank.statement.import"]._is_acap_file(acap_file)
        )

    def test_003_bdr_statement_in_state_open(self):
        """
        Check acap statement file is passed to the "Open" state
        """
