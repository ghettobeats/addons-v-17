...
class HelpdeskTicketType(models.Model):
    _name = 'helpdesk.second.ticket.type'
...
class HelpdeskSLAStatus(models.Model):
    _name = 'helpdesk.second.sla.status'
    _description = "Ticket SLA Status"
    _table = 'helpdesk_second_sla_status'
    _order = 'deadline ASC, sla_stage_id'
    _rec_name = 'sla_id'

    ticket_id = fields.Many2one('helpdesk.second.ticket', string='Ticket', required=True, ondelete='cascade', index=True)
    sla_id = fields.Many2one('helpdesk.second.sla', required=True, ondelete='cascade')
    sla_stage_id = fields.Many2one('helpdesk.second.stage', related='sla_id.stage_id', store=True)  # need to be stored for the search in `_sla_reach`
...
class HelpdeskTicket(models.Model):
    ...
    def default_get(self, fields):
        ...
        if result.get('team_id') and fields:
            team = self.env['helpdesk.second.team'].browse(result['team_id'])
    ...
    def _default_team_id(self):
        team_id = self.env['helpdesk.second.team'].search([('member_ids', 'in', self.env.uid)], limit=1).id
        if not team_id:
            team_id = self.env['helpdesk.second.team'].search([], limit=1).id
        return team_id
    ...
    team_id = fields.Many2one('helpdesk.second.team', string='Helpdesk Team', default=_default_team_id, index=True)
    use_sla = fields.Boolean(related='team_id.use_sla')
    description = fields.Html()
    active = fields.Boolean(default=True)
    ticket_type_id = fields.Many2one('helpdesk.second.ticket.type', string="Type")
    tag_ids = fields.Many2many('helpdesk.second.tag', string='Tags')
    ...
    partner_ticket_ids = fields.Many2many('helpdesk.second.ticket', compute='_compute_partner_ticket_count', string="Partner Tickets")
    ...
    stage_id = fields.Many2one(
        'helpdesk.second.stage', string='Stage', compute='_compute_user_and_stage_ids', store=True,
    ...
    # SLA relative
    sla_ids = fields.Many2many('helpdesk.second.sla', 'helpdesk_second_sla_status', 'ticket_id', 'sla_id', string="SLAs", copy=False)
    sla_status_ids = fields.One2many('helpdesk.second.sla.status', 'ticket_id', string="SLA Status")
    ...
    def create(self, list_value):
        ...
        teams = self.env['helpdesk.second.team'].browse([vals['team_id'] for vals in list_value if vals.get('team_id')])
    ...
    def write(self, vals):
        ...
        if vals.get('stage_id'):
            if self.env['helpdesk.second.stage'].browse(vals.get('stage_id')).is_close:
    ...
    def _sla_apply(self, keep_reached=False):
        """ Apply SLA to current tickets: erase the current SLAs, then find and link the new SLAs to each ticket.
            ...
            :returns recordset of new helpdesk.second.sla.status applied on current tickets
        """
        # get SLA to apply
        ...
        return self.env['helpdesk.second.sla.status'].create(sla_status_value_list)
    ...
    def _sla_find(self):
        """ Find the SLA to apply on the current tickets
            ...
            :rtype : dict {<helpdesk.second.ticket>: <helpdesk.second.sla>}
        """
        tickets_map = {}
        sla_domain_map = {}
        ...
        for ticket in self:
            if ticket.team_id.use_sla:  # limit to the team using SLA
                ...
                tickets_map.setdefault(key, self.env['helpdesk.second.ticket'])
        ...
        for key, tickets in tickets_map.items():  # only one search per ticket group
            ...
            slas = self.env['helpdesk.second.sla'].search(domain)
    ...
    def _sla_reach(self, stage_id):
        ...
        """
        stage = self.env['helpdesk.second.stage'].browse(stage_id)
        stages = self.env['helpdesk.second.stage'].search([('sequence', '<=', stage.sequence), ('team_ids', 'in', self.mapped('team_id').ids)])  # take previous stages
        self.env['helpdesk.second.sla.status'].search([
    ...
    def action_open_helpdesk_ticket(self):
        self.ensure_one()
        action = self.env["ir.actions.actions"]._for_xml_id("helpdesk_second.helpdesk_ticket_action_main_tree")
        action.update({
    ...
    def rating_apply(self, rate, token=None, feedback=None, subtype_xmlid=None):
        return super(HelpdeskTicket, self).rating_apply(rate, token=token, feedback=feedback, subtype_xmlid="helpdesk_second.mt_ticket_rated")
