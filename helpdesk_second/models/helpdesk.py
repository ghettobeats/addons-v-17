...
class HelpdeskTeam(models.Model):
    ...
    def _default_stage_ids(self):
        default_stage = self.env['helpdesk.second.stage'].search([('name', '=', _('New'))], limit=1)
        if not default_stage:
            default_stage = self.env['helpdesk.second.stage'].create({
                'name': _("New"),
                'sequence': 0,
    ...
    stage_ids = fields.Many2many(
        'helpdesk.second.stage', relation='team_stage_second_rel', string='Stages',
    ...
    visibility_member_ids = fields.Many2many('res.users', 'helpdesk_second_visibility_team', string='Team Visibility', domain=lambda self: self._default_domain_member_ids(),
    ...
    from_stage_ids = fields.Many2many('helpdesk.second.stage', relation='team_stage_second_auto_close_from_rel',
        string='In Stages',
    ...
    to_stage_id = fields.Many2one('helpdesk.second.stage',
    ...
    def _compute_assign_stage_id(self):
        stages_dict = {stage['id']: 1 if stage['is_close'] else 2 for stage in self.env['helpdesk.second.stage'].search_read([('id', 'in', self.stage_ids.ids), '|', ('is_close', '=', True), ('fold', '=', True)], ['id', 'is_close'])}
        for team in self:
            stage_ids = sorted([
    ...
    def _compute_sla_policy_count(self):
        sla_data = self.env['helpdesk.second.sla'].read_group([('team_id', 'in', self.ids)], ['team_id'], ['team_id'])
    ...
    def _check_sla_group(self):
        ...
        if sla_teams:
            self.env['helpdesk.second.sla'].with_context(active_test=False).search([
                ('team_id', 'in', sla_teams.ids), ('active', '=', False),
            ]).write({'active': True})
        if non_sla_teams:
            self.env['helpdesk.second.sla'].search([('team_id', 'in', non_sla_teams.ids)]).write({'active': False})
    ...
    def retrieve_dashboard(self):
        ...
        user_uses_sla = self.user_has_groups('helpdesk.group_use_sla') and\
            bool(self.env['helpdesk.second.team'].search([('use_sla', '=', True), ('member_ids', 'in', self._uid)]))
        ...
        if self.env['helpdesk.second.team'].search([('use_rating', '=', True), ('member_ids', 'in', self._uid)]):
    ...
    def _action_view_rating(self, period=False, only_my_closed=False):
        ...
        action = self.env["ir.actions.actions"]._for_xml_id("helpdesk_second.rating_rating_action_helpdesk")
    ...
    def action_view_ticket(self):
        action = self.env["ir.actions.actions"]._for_xml_id("helpdesk_second.helpdesk_ticket_action_team")
    ...
    def action_view_sla_policy(self):
        self.ensure_one()
        action = self.env["ir.actions.actions"]._for_xml_id("helpdesk_second.helpdesk_sla_action")
        if self.sla_policy_count == 1:
            action.update({
                'view_mode': 'form',
                'res_id': self.env['helpdesk.second.sla'].search([('team_id', '=', self.id)], limit=1).id,
                'views': [(False, 'form')],
            })
        action.update({
    ...
    def _determine_stage(self):
        """ Get a dict with the stage (per team) that should be set as first to a created ticket
            ...
            :rtype : dict (key=team_id, value=record of helpdesk.second.stage)
        """
        result = dict.fromkeys(self.ids, self.env['helpdesk.second.stage'])
        for team in self:
            result[team.id] = self.env['helpdesk.second.stage'].search([('team_ids', 'in', team.id)], order='sequence', limit=1)
        return result
    ...
    def _cron_auto_close_tickets(self):
        teams = self.env['helpdesk.second.team'].search_read(
            domain=[
                ('auto_close_ticket', '=', True),
                ('auto_close_day', '>', 0),
                ('to_stage_id', '!=', False)],
...
class HelpdeskStage(models.Model):
    ...
    team_ids = fields.Many2many(
        'helpdesk.second.team', relation='team_stage_second_rel', string='Helpdesk Teams',
