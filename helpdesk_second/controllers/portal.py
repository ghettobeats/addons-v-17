...
class CustomerPortal(portal.CustomerPortal):
    ...
    def _prepare_home_portal_values(self, counters):
        ...
        if 'ticket_count' in counters:
            values['ticket_count'] = (
                request.env['helpdesk.second.ticket'].search_count([])
                if request.env['helpdesk.second.ticket'].check_access_rights('read', raise_exception=False)
                else 0
            )
        return values
    ...
    @http.route(['/my/tickets2', '/my/tickets2/page/<int:page>'], type='http', auth="user", website=True)
    def my_helpdesk_tickets(self, page=1, date_begin=None, date_end=None, sortby=None, filterby='all', search=None, groupby='none', search_in='content', **kw):
        ...
        if filterby in ['last_message_sup', 'last_message_cust']:
            ...
            messages = request.env['mail.message'].search_read([('model', '=', 'helpdesk.second.ticket'), ('subtype_id', '=', discussion_subtype_id)], fields=['res_id', 'author_id'], order='date desc')
            last_author_dict = {}
            for message in messages:
                if message['res_id'] not in last_author_dict:
                    last_author_dict[message['res_id']] = message['author_id'][0]

            ticket_author_list = request.env['helpdesk.second.ticket'].search_read(fields=['id', 'partner_id'])
        ...
        # pager
        tickets_count = request.env['helpdesk.second.ticket'].search_count(domain)
        pager = portal_pager(
            url="/my/tickets2",
        ...
        tickets = request.env['helpdesk.second.ticket'].search(domain, order=order, limit=self._items_per_page, offset=pager['offset'])
        request.session['my_tickets_history'] = tickets.ids[:100]

        if groupby == 'stage':
            grouped_tickets = [request.env['helpdesk.second.ticket'].concat(*g) for k, g in groupbyelem(tickets, itemgetter('stage_id'))]
        else:
        ...
        return request.render("helpdesk.second.portal_helpdesk_ticket", values)

    @http.route([
        "/helpdesk/ticket/<int:ticket_id>",
        "/helpdesk/ticket/<int:ticket_id>/<access_token>",
        '/my/ticket2/<int:ticket_id>',
        '/my/ticket2/<int:ticket_id>/<access_token>'
    ], type='http', auth="public", website=True)
    def tickets_followup(self, ticket_id=None, access_token=None, **kw):
        try:
            ticket_sudo = self._document_check_access('helpdesk.second.ticket', ticket_id, access_token)
        except (AccessError, MissingError):
            return request.redirect('/my')

        values = self._ticket_get_page_view_values(ticket_sudo, access_token, **kw)
        return request.render("helpdesk.second.tickets_followup", values)

    @http.route([
        '/my/ticket2/close/<int:ticket_id>',
        '/my/ticket2/close/<int:ticket_id>/<access_token>',
    ], type='http', auth="public", website=True)
    def ticket_close(self, ticket_id=None, access_token=None, **kw):
        ...
        return request.redirect('/my/ticket2/%s/%s' % (ticket_id, access_token or ''))
