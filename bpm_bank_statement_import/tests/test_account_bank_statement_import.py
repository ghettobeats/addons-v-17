...
class AccountBankStatementImportTest(AccountTestInvoicingCommon):
    def test_001_bpm_statement_import(self):
        """
        Check bpm statement file is processed correctly
        """

        bpm_file_path = get_module_resource(
            "bpm_bank_statement_import", "bpm_stmt_file", "bpm_statement.xls"
        )

        bpm_file = base64.b64encode(open(bpm_file_path, "rb").read())
        ...
        dop_currency_id = self.env.ref("base.USD")
        ...
        import_wizard = (
            ...
            .create(
                {
                    "attachment_ids": [
                        (0, 0, {"name": "bpm_stmt.xls", "datas": bpm_file})
                    ]
                }
            )
        )
    ...
    def test_002_bpm_statement_file_validation(self):
        """
        Check bpm file format validation works correctly
        """

        bpm_file_path = get_module_resource(
            "bpm_bank_statement_import", "bpm_stmt_file", "bpm_statement.xls"
        )
        bpm_file = xlrd.open_workbook(bpm_file_path)
        self.assertTrue(
            self.env["account.bank.statement.import"]._is_bpm_file(bpm_file)
        )

        bad_bpm_file_path = get_module_resource(
            "bpm_bank_statement_import", "bpm_stmt_file", "bad_bpm_statement.xls"
        )
        bad_bpm_file = xlrd.open_workbook(bad_bpm_file_path)
        self.assertFalse(
            self.env["account.bank.statement.import"]._is_bpm_file(bad_bpm_file)
        )

    def test_003_bpm_statement_in_state_open(self):
        """
        Check bpm statement file is passed to the "Open" state
        """
