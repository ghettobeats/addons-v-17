...
class InvoicePrintBankChequeWizard(models.TransientModel):
    ...
    amount_in_words = fields.Char("Amount In Words", default=_get_amount_in_words)
    amount_in_words_line2 = fields.Char(
    ...
    def _check_cheque_attributes(self):
        ...
        if self.cheque_book_id:
            for (
                bank_cheque_attr
            ) in self.cheque_book_id.bank_cheque_id.cheque_attribute_line_ids.filtered(
        ...
        if self.amount_in_words:
            if self.cheque_book_id.bank_cheque_id.max_char_in_line1:
                ...
                for word in raw_str.split(" "):
                    if (
                        total_word + len(word)
                        <= self.cheque_book_id.bank_cheque_id.max_char_in_line1
                    ):
