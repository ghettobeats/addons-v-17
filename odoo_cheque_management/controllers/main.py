...
class BankChequeManagement(http.Controller):
    @http.route(
        '/bank/cheque/<model("res.bank"):bank_cheque_id>',
        type="http",
        auth="user",
        website=True,
    )
    def bank_cheque_management(self, bank_cheque_id, **post):
        ...
        return request.render(
            "odoo_cheque_management.bank_cheque_management_template", values
        )
    ...
    def bank_cheque_update_attrs(self, **post):
        ...
        if post.get("cheque_attribute_line_id"):
            is_updated = (
                request.env["bank.cheque.attribute.line"]
                .browse(int(post.get("cheque_attribute_line_id")))
                .write(
                    {
                        "top_displacement": int(post.get("y1", 0)),
                        "left_displacement": int(post.get("x1"))
                        if post.get("x1")
                        else 0,
        ...
        values = {
            "bank_cheque_obj": request.env["res.bank"].browse(
                int(post.get("bank_cheque_id"))
            )
            if post.get("bank_cheque_id")
            else False,
        }
        if is_updated:
            values.update(
                {
                    "updated_cheque_attribute_line_id": int(
                        post.get("cheque_attribute_line_id")
                    )
                }
            )
    ...
    @http.route(
        '/bank/cheque/preview/<model("res.bank"):bank_cheque_id>',
        type="http",
        auth="user",
        website=True,
    )
