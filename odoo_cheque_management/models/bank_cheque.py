...
class BankCheque(models.Model):
    ...
    def redirect_to_bank_cheque_page(self):
        ...
        if not self.cheque_attribute_line_ids:
            raise UserError(
                _(
                    "First you have to set bank cheque attributes. "
...
class BankChequeAttributeLine(models.Model):
    ...
    bank_cheque_id = fields.Many2one("res.bank", string="Bank Cheque", required=True)
    ...
    def onchange_name(self):
        if (
            self.name
            and self.name.attribute == "cheque_date"
            and not self.letter_spacing
        ):
...
class BankChequeAttribute(models.Model):
    ...
    demo_data_date = fields.Date("Demo Date For Preview", default=fields.Date.today())
    date_format = fields.Selection(
        [("ddMMyyyy", "DD MM YYYY"), ("MMddyyyy", "MM DD YYYY")],
        "Date Format",
        default="ddMMyyyy",
    )
    ...
    def set_cheque_book_number(self):
        for rec in self:
            if rec.cheque_book_leaves and rec.initial_cheque_number:
                rec.last_cheque_number = (
                    rec.initial_cheque_number + rec.cheque_book_leaves - 1
                )

    def create_cheque_leaves(self):
        for rec in self:
            if (
                rec.cheque_book_leaves
                and rec.initial_cheque_number
                and rec.last_cheque_number
            ):
                i = rec.initial_cheque_number
                for i in range(
                    rec.initial_cheque_number, rec.last_cheque_number + 1, 1
                ):
                    vals = {"cheque_number": i, "bank_cheque_book_id": rec.id}
...
class IssuesBankChequeHistory(models.Model):
    ...
    state = fields.Selection(
        [("blank", "Blank"), ("printed", "Printed"), ("cancelled", "Cancelled")],
        "State",
        default="blank",
    )
        ...
        for record in self:
            ...
            if len(already_exist) > 1:
                raise ValidationError(
                    _(
                        "Cheque number %s is not valid. It has been already used. "
                        "Please use diffrent cheque number."
                    )
    ...
    def on_change_cheque_number(self):
        if (
            self.cheque_number
            and self.bank_cheque_book_id
            and self.cheque_number
            not in range(
                self.bank_cheque_book_id.initial_cheque_number,
                self.bank_cheque_book_id.last_cheque_number + 1,
                1,
            )
        ):
            raise UserError(
                _(
                    "Invalid cheque number. Cheque number should be in range %s to %s (including last number)."
                )
                % (
                    self.bank_cheque_book_id.initial_cheque_number,
        ...
        for rec in self:
            if rec.model == "res.bank" and bodies:
                bodies = [
                    bytes(
                        bodies[0]
                        .replace(b'class="container"', b'class="" style="margin:0px"')
                        .replace(b'class="article o_report_layout_clean"', b'class=""')
                    )
                ]
